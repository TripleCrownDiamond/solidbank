...
@if($compact)
    ...
    <div class="financial-card flex-none w-64 p-3 rounded-lg shadow-xl hover:shadow-2xl transition-shadow duration-300 border border-brand-secondary/20 dark:border-brand-secondary/30">
        <div class="flex items-center justify-between mb-2">
            <div class="w-10 h-10 rounded-lg flex items-center justify-center shadow-lg border border-brand-secondary/30 dark:border-brand-secondary/40 bg-brand-primary">
                @if($type === 'card')
            ...
            <div class="text-right">
                ...
                <p class="text-xs text-brand-secondary dark:text-brand-secondary/80">{{ $item->currency }}</p>
                @else
                    <p class="text-lg font-bold text-black dark:text-white">{{ \App\Helpers\NumberHelper::formatCrypto($item->balance) }}</p>
                <p class="text-xs text-brand-secondary dark:text-brand-secondary/80">{{ $item->cryptocurrency->symbol }}</p>
                @endif
            </div>
        </div>
        <div>
            ...
            <p class="text-brand-secondary dark:text-brand-secondary/80 font-mono text-sm">
            ...
            <p class="text-brand-secondary dark:text-brand-secondary/80 text-sm">
...
@else
    ...
    <div class="relative">
        <!-- Card/Wallet Design -->
        <div class="financial-card rounded-xl p-6 text-black dark:text-white shadow-xl hover:shadow-2xl transform hover:scale-105 transition-all duration-300 border border-brand-secondary/20 dark:border-brand-secondary/30">
            <!-- Header -->
            <div class="flex justify-between items-start mb-6">
                <div>
                    @if($type === 'card')
                        <p class="text-brand-secondary dark:text-brand-secondary/80 text-sm font-medium">{{ getAppName() }}</p>
                ...
                <div class="text-right">
                    <p class="text-brand-secondary dark:text-brand-secondary/80 text-sm">{{ __('common.balance') }}</p>
                    @if($type === 'card')
            ...
            <div class="mb-6">
                @if($type === 'card')
                    <p class="text-brand-secondary dark:text-brand-secondary/80 text-sm mb-1">{{ __('common.card_number') }}</p>
                ...
                @else
                    <p class="text-brand-secondary dark:text-brand-secondary/80 text-sm mb-1">{{ __('common.wallet_address') }}</p>
            ...
            <div class="flex justify-between items-end">
                @if($type === 'card')
                    <div>
                        <p class="text-brand-secondary dark:text-brand-secondary/80 text-xs">{{ __('common.card_holder') }}</p>
                        <p class="text-sm font-medium text-black dark:text-white">{{ $item->card_holder_name }}</p>
                    </div>
                    <div class="text-right">
                        <p class="text-brand-secondary dark:text-brand-secondary/80 text-xs">{{ __('common.expires') }}</p>
                ...
                @else
                    <div>
                        <p class="text-brand-secondary dark:text-brand-secondary/80 text-xs">{{ __('common.cryptocurrency') }}</p>
                        <p class="text-sm font-medium text-black dark:text-white">{{ $item->cryptocurrency->name }}</p>
                    </div>
                    <div class="text-right">
                        <p class="text-brand-secondary dark:text-brand-secondary/80 text-xs">{{ __('common.network') }}</p>
            ...
            <!-- Additional Details -->
            @if($showDetails)
                <div class="mt-4 pt-4 border-t border-brand-secondary/30">
                    <div class="flex justify-between items-center">
                        @if($type === 'card')
                            <div>
                                <p class="text-brand-secondary dark:text-brand-secondary/80 text-xs">{{ __('common.cvv') }}</p>
                                <p class="text-sm font-medium text-black dark:text-white">{{ $item->cvv }}</p>
                            </div>
                        @elseif($item->private_key)
                            <div>
                                <p class="text-brand-secondary dark:text-brand-secondary/80 text-xs">{{ __('common.private_key') }}</p>
        ...
        <!-- Account Info -->
        <div class="mt-3 p-3 rounded-lg bg-brand-secondary/5 dark:bg-brand-secondary/10">
            <div class="flex justify-between items-center text-sm">
                <span class="text-brand-secondary dark:text-brand-secondary/70">{{ __('common.linked_to_account') }}:</span>
                @if($type === 'card')
                    <span class="font-medium text-black dark:text-white">{{ $item->account->account_number }}</span>
                @elseif($item->user && $item->user->account)
                    <span class="font-medium text-black dark:text-white">{{ $item->user->account->account_number }}</span>
                @endif
            </div>
            <div class="flex justify-between items-center text-sm mt-1">
                <span class="text-brand-secondary dark:text-brand-secondary/70">{{ __('common.account_type') }}:</span>
                @if($type === 'card')
                    <span class="font-medium text-black dark:text-white">{{ trans('common.account_types.' . $item->account->type) ?: ucfirst(strtolower($item->account->type)) }}</span>
                @elseif($item->user && $item->user->account)
                    <span class="font-medium text-black dark:text-white">{{ trans('common.account_types.' . $item->user->account->type) ?: ucfirst(strtolower($item->user->account->type)) }}</span>
                @endif
            </div>
        </div>
            ...
            <div class="mt-3 flex gap-2" x-data="{ copied: false }">
                ...
                @if($type === 'card')
                    <button x-on:click="navigator.clipboard.writeText('{{ $item->card_number }}').then(() => { copied = true; setTimeout(() => copied = false, 2000); })"
                            class="px-3 py-2 bg-brand-primary hover:bg-brand-primary/80 text-white rounded-lg transition-colors duration-200"
                ...
                @if($type === 'card')
                    <button onclick="Livewire.dispatch('toggleCardDetails', { cardId: {{ $item->id }} })"
                            class="px-3 py-2 bg-brand-secondary hover:bg-brand-secondary/80 text-white rounded-lg transition-colors duration-200"
